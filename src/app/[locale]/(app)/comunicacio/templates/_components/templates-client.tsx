...
export function TemplatesClient({ initialTemplates }: { initialTemplates: EmailTemplate[] }) {
  ...
  const t = useTranslations('TemplatesPage'); // ✅ Cridem el hook
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-[280px_1fr_280px] gap-6 min-h-0">
          ...
          {selectedTemplate ? (
            <div className="flex flex-col gap-4">
              ...
              <Input placeholder={t('templateNamePlaceholder')} value={selectedTemplate.name} onChange={(e) => setSelectedTemplate(t => t ? {...t, name: e.target.value} : null)} />
              <Input placeholder={t('subjectPlaceholder')} value={selectedTemplate.subject} onChange={(e) => setSelectedTemplate(t => t ? {...t, subject: e.target.value} : null)} />
              </div>
              ...
              <h3 className="font-semibold px-2">{t('contentTitle')}</h3>
                  <div className="flex items-center gap-1 p-1 rounded-lg bg-black/20">
                    <Button variant={editorView === 'code' ? 'secondary' : 'ghost'} size="sm" className="h-7 px-3" onClick={() => setEditorView('code')}><Code className="w-4 h-4 mr-2"/>{t('codeButton')}</Button>
                    <Button variant={editorView === 'preview' ? 'secondary' : 'ghost'} size="sm" className="h-7 px-3" onClick={() => setEditorView('preview')}><Eye className="w-4 h-4 mr-2"/>{t('previewButton')}</Button>
                  </div>
                {editorView === 'code' ? (
          ...
          {/* Columna Variables */}
          <div className="glass-card flex flex-col overflow-hidden">
            <div className="p-4 border-b border-border"><h3 className="font-semibold flex items-center gap-2"><Variable className="w-4 h-4 text-primary"/>{t('detectedVariablesTitle')}</h3></div>
            <div className="p-4 space-y-3 overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-2">
                {detectedVariables.length > 0 ? detectedVariables.map(v => (
                  <Button key={v} variant="outline" size="sm" onClick={() => { navigator.clipboard.writeText(`{{${v}}}`); toast.success(t('toastCopiedTitle'), { description: t('toastCopiedDescription', {variable: `{{${v}}}`}) }); }}>{`{{${v}}}`}</Button>
      ...
      <AlertDialog open={!!templateToDelete} onOpenChange={() => setTemplateToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteTemplate} disabled={isDeleting} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">{isDeleting ? t('deletingButton') : t('confirmDeleteButton')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
