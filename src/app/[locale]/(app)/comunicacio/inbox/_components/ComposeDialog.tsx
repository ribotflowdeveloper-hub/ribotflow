...
import { Loader2, Send, FileText, Variable, User, Mail, Search, Lock, Briefcase } from 'lucide-react';
...
import type { Contact, Template, Pipeline } from '@/types/db';
...
export const ComposeDialog = (props: ComposeDialogProps) => {
    ...
    const { open, onOpenChange, onEmailSent, initialData, templates, contacts, limitStatus, pipelines } = props;
    ...
    // Lògica per desactivar el selector (si no hi ha destinatari)
    const canSelectPipeline = (recipientMode === 'contact' && !!selectedContactId) ||
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
                    <div className="lg:col-span-2 flex flex-col gap-4 min-h-0">
                        ...
                        <div className="flex-1 flex flex-col gap-2 min-h-0">
                            ...
                            {selectedTemplate ? (
                                ...
                                // ✅ 4. CORRECCIÓ DE LAYOUT (la que et faltava)
                            ) : (
                                // Embolcallem l'editor en un div per controlar el flex i l'overflow
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-6 bg-muted/30 p-4 rounded-lg overflow-y-auto">
                        <div className="space-y-2">
                            <Label className="flex items-center gap-2 font-semibold"><User className="w-4 h-4" />{t('recipientLabel')}</Label>

                            {/* ✅ 5. IMPLEMENTACIÓ DELS TABS */}
                            <Tabs
                                value={recipientMode}
                                onValueChange={(value) => setRecipientMode(value as 'contact' | 'manual')}
                                className="w-full"
                            >
                                <TabsList className="grid w-full grid-cols-2">
                                    <TabsTrigger value="contact">{t_toast('existingContact')}</TabsTrigger>
                                    <TabsTrigger value="manual">{t_toast('manualEmail')}</TabsTrigger>
                                </TabsList>
                                ...
                                <TabsContent value="manual" className="space-y-2 pt-2">
                                    <div className="relative">
                                        ...
                                        <Input
                                            id="manual-email"
                                            type="email"
                                            placeholder={t_toast('manualEmailPlaceholder')}
                                            value={manualEmail}
                                            onChange={(e) => setManualEmail(e.target.value)}
                                            className="pl-9"
                                        />
                                    </div>
                                    <p className="text-xs text-muted-foreground px-1">{t_toast('manualEmailDescription')}</p>
                                </TabsContent>
                            </Tabs>

                            {/* ✅ 5. CANVI DE CHECKBOX A SELECTOR */}
                            <div className="space-y-2 pt-4">
                                <Label htmlFor="pipeline-select" className="flex items-center gap-2 font-semibold">
                                    <Briefcase className="w-4 h-4" />
                                    {t('createOpportunityInPipeline')}
                                </Label>
                                <Select
                                    // Convertim null a 'null' (string) per al Select
                                    value={selectedPipelineId === null ? 'null' : String(selectedPipelineId)}
                                    onValueChange={(value) => {
                                        setSelectedPipelineId(value === 'null' ? null : Number(value))
                                    }}
                                    disabled={!canSelectPipeline || pipelines.length === 0}
                                >
                                    <SelectTrigger id="pipeline-select">
                                        <SelectValue placeholder={t('selectPipelinePlaceholder')} />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="null">{t('doNotCreateOpportunity')}</SelectItem>
                                ...
                                {pipelines.length === 0 && canSelectPipeline && (
                                    <p className="text-xs text-muted-foreground px-1">{t('noPipelinesFound')}</p>
                                )}
                            </div>
