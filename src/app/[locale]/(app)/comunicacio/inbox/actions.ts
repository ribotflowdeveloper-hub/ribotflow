...
  try {
    await inboxService.assignTicket(supabase, ticketId, dealId, activeTeamId);
    revalidatePath("/comunicacio/inbox");
...
  try {
    await inboxService.addToBlacklist(supabase, emailToBlock, user.id, activeTeamId);
    revalidatePath("/comunicacio/inbox");
    return { success: true, message: `${emailToBlock} ha estat afegit a la llista negra.` };
...
    const ticket = await inboxService.getTicketById(supabase, ticketId, user.id, activeTeamId);
    
    if (!ticket) {
      return { data: null, error: "No s'ha pogut trobar el correu (accés denegat o no existeix)." };
    }
...
  try {
    const tickets = await inboxService.fetchTicketsForSupplierContacts(supabase, supplierId, activeTeamId);
    // Fem un 'cast' per assegurar que les dades retornades compleixen amb la nostra interface.
    return (tickets as TicketForSupplier[]) || [];
  } catch (error: unknown) {
    console.error("Error fetching tickets for supplier contacts (action):", error);
    return [];
  }
}
...
    try {
        // 1. Cridem al servei
        const { data, contactCreated } = await inboxService.prepareNetworkContact(
            supabase,
            recipientTeamId,
            projectId,
            activeTeam
