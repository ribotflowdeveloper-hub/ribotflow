            ...
            <DialogHeader>
                ...
                {localProfile && (
                    <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto">
                        ...
                        <div className="mt-1 flex items-center gap-4">
                                ...
                                <Button asChild variant="outline">
                                    <label htmlFor="logo-upload" className="cursor-pointer flex items-center gap-2">
                                        {isUploading ? <Loader2 className="animate-spin" /> : <Upload className="w-4 h-4" />} {t('companyProfileDialog.uploadButton')}
                                    </label>
                                </Button>
                        ...
                        <div><Label htmlFor="company_name">{t('companyProfileDialog.nameLabel')}</Label><Input id="company_name" value={localProfile?.company_name || ''} onChange={(e) => setLocalProfile(p => p ? ({...p, company_name: e.target.value}) : null)} /></div>
                        <div><Label htmlFor="company_tax_id">{t('companyProfileDialog.taxIdLabel')}</Label><Input id="company_tax_id" value={localProfile?.company_tax_id || ''} onChange={(e) => setLocalProfile(p => p ? ({...p, company_tax_id: e.target.value}) : null)} /></div>
                        <div><Label htmlFor="company_address">{t('companyProfileDialog.addressLabel')}</Label><Input id="company_address" value={localProfile?.company_address || ''} onChange={(e) => setLocalProfile(p => p ? ({...p, company_address: e.target.value}) : null)} /></div>
                        <div><Label htmlFor="company_email">{t('companyProfileDialog.emailLabel')}</Label><Input id="company_email" type="email" value={localProfile?.company_email || ''} onChange={(e) => setLocalProfile(p => p ? ({...p, company_email: e.target.value}) : null)} /></div>
                        <div><Label htmlFor="company_phone">{t('companyProfileDialog.phoneLabel')}</Label><Input id="company_phone" value={localProfile?.company_phone || ''} onChange={(e) => setLocalProfile(p => p ? ({...p, company_phone: e.target.value}) : null)} /></div>
                    </div>
                )}
                <DialogFooter>
