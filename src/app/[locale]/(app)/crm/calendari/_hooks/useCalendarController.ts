...
export const useCalendarController = ({
  ...
  // Es torna a cridar sempre que canviem de vista o data.
  const handleDataFetch = useCallback(async (currentDate: Date, currentView: View) => {
    setIsLoading(true); // Mostrem loader a la UI

    try {
      // Calculem el rang de dates segons la vista
      const { start, end } = getDateRange(currentDate, currentView);

      // Convertim les dates a ISO per enviar-les al backend
      const startDateString = start.toISOString();
      const endDateString = end.toISOString();

      // Cridem l’acció de fetch que ens retorna totes les dades
      const updatedData = await fetchCalendarDataAction(startDateString, endDateString);

      if (updatedData.error) {
        // Si hi ha error de backend, el notifiquem i buidem dades
        toast.error("Error carregant dades del calendari.", { description: updatedData.error });
        setTasks([]); setQuotes([]); setSentEmails([]); setReceivedEmails([]);
        return;
      }
      ...
      // Sigui quin sigui el resultat, deixem d’estar en estat de càrrega
      setIsLoading(false);
    }
  }, [fetchCalendarDataAction]);
