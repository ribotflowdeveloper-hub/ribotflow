// src/app/[locale]/(app)/_components/AppClientLayout.tsx
...
export function AppClientLayout({
  ...
  // ✅ CORRECCIÓ 1: Ja no necessitem 'toggleChatbot' aquí
  ...
  const {
    ...
    handleNavigation,
    // ✅ CORRECCIÓ 2: Recuperem el nom original de la funció del hook
    handleMainMenuClick,
  } = useAppNavigation()
  ...
  // ✅ CORRECCIÓ 3: Eliminem la funció 'handleMainMenuClick' wrapper
  // Ja no és necessària, la lògica es gestionarà dins de 'MainSidebar'

  const safeAiLimitStatus: UsageCheckResult = aiLimitStatus ?? {
    allowed: false,
    current: 0,
    max: 0,
  }
  ...
  return (
    <div className="h-screen w-screen flex flex-col lg:flex-row bg-background text-foreground overflow-hidden">
      <MainSidebar
        // ✅ CORRECCIÓ 4: Passem la funció original del hook 'useAppNavigation'
        // Aquesta funció SÍ té la signatura correcta: (item: NavItem) => void
        ...
        aiLimitStatus={safeAiLimitStatus}
      />
