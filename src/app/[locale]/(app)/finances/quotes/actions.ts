...
export type QuoteWithContact = Quote & {
    // ✅ MODIFICACIÓ 1: Afegim l'ID al tipus de l'objecte 'contacts'
    contacts: {
        id: number; // <-- AFEGIT: El client necessita aquest ID per l'enllaç
        nom: string;
        empresa: string | null;
    } | null;
};
...
export async function fetchPaginatedQuotes(
    ...
    console.log(
        "Calling RPC 'search_paginated_quotes' with params:",
        rpcParams,
    );
    ...
    const mappedData: QuoteWithContact[] = data.map(
        (row): QuoteWithContact | null => {
            ...
            return {
                ...quoteBase,
                // ✅ MODIFICACIÓ 2: Assignem l'ID del contacte a l'objecte 'contacts'
                contacts: row.contact_id
                    ? {
                        id: row.contact_id, // <-- AFEGIT: Passem l'ID de la fila
...
export async function deleteQuoteAction(
    quoteId: QuoteId,
): Promise<ActionResult> { // <-- Retorna ActionResult
    const session = await validateUserSession();
    if ("error" in session) {
    ...
    if (itemsError) {
        ...
        return {
            success: false,
            message: "No s'han pogut eliminar els detalls del pressupost.",
        };
    }
    ...
    if (quoteError) {
        ...
        return {
            success: false,
            message: "No s'ha pogut eliminar el pressupost.",
        };
    }
