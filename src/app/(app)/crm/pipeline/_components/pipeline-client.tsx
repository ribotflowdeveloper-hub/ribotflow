...
// --- Sub-component: Targeta d'Oportunitat (Disseny Original) ---
        ...
        >
          ...
          <div className="flex justify-between items-center mt-3 pt-2 border-t border-white/5 text-xs">
            ...
            <span className="text-muted-foreground flex items-center gap-1">
              <Calendar className="w-3 h-3"/> {opportunity.close_date ? new Date(opportunity.close_date).toLocaleDateString('ca-ES', { day: '2-digit', month: '2-digit' }) : '-'}
            </span>
          </div>
        </div>
      )}
...
// --- Sub-component: Columna d'Etapa (Disseny Original) ---
const StageColumn: React.FC<{ stage: Stage; opportunities: Opportunity[]; onEditOpportunity: (opportunity: Opportunity) => void; onAddClick: () => void; }> = ({ stage, opportunities, onEditOpportunity, onAddClick }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-muted/20 rounded-xl overflow-hidden w-80 flex-shrink-0">
...
// --- Component Principal del Pipeline ---
        ...
        <div className="flex-1 overflow-y-auto pr-2 -mr-4">
            <Accordion type="multiple" defaultValue={stages.map(s => s.id)} className="w-full space-y-4">
                {stages.map(stage => {
                    ...
                    const totalValue = opportunities.reduce((sum, op) => sum + (op.value || 0), 0);
                    return (
                        <AccordionItem key={stage.id} value={stage.id} className={cn("bg-muted/20 rounded-xl overflow-hidden border-t-4", stageColors[stage.name] || 'border-gray-500')}>
                            <AccordionTrigger className="px-4 py-3 hover:no-underline">
                                <div className="w-full flex justify-between items-center">
                                    <div>
                                        <h3 className="font-bold text-lg text-foreground">{stage.name}</h3>
                                        <p className="text-xs text-muted-foreground text-left">{opportunities.length} oportunitat{opportunities.length !== 1 ? 's' : ''} • €{totalValue.toLocaleString('ca-ES')}</p>
                                    </div>
                                    <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); handleOpenCreateDialog(stage.name)}} className="mr-2"><Plus className="w-4 h-4 mr-2" />Afegir</Button>
                                </div>
                            </AccordionTrigger>
                                ...
                                <div className="space-y-2">
                                    {opportunities.length > 0 ? (
                                        opportunities.map(op => (
                                            <div key={op.id} onDoubleClick={() => handleOpenEditDialog(op)} className="grid grid-cols-2 md:grid-cols-5 gap-4 px-3 py-2.5 items-center rounded-md hover:bg-background/80 cursor-pointer">
                                                <div className="font-semibold col-span-2">{op.name}</div>
                                                <div className="flex items-center text-sm text-muted-foreground"><User className="w-4 h-4 mr-2" />{op.contacts?.nom || 'N/A'}</div>
                                                <div className="flex items-center text-sm text-muted-foreground"><Euro className="w-4 h-4 mr-2" />{op.value?.toLocaleString('ca-ES') || '0'} €</div>
                                                <div className="flex justify-end items-center text-xs text-muted-foreground gap-2">
                                                    <Calendar className="w-3 h-3"/>
                                                    {op.close_date ? new Date(op.close_date).toLocaleDateString('ca-ES', { day: '2-digit', month: '2-digit' }) : '-'}
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                                        <p className="text-center text-sm text-muted-foreground p-4">No hi ha oportunitats en aquesta etapa.</p>
                                    )}
                                </div>
                            </AccordionContent>
                        </AccordionItem>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="h-full flex flex-col">
        <div className="flex justify-between items-center mb-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <div className="bg-muted p-1 rounded-lg">
                <Button variant={viewMode === 'columns' ? 'secondary' : 'ghost'} size="icon" onClick={() => setViewMode('columns')}><LayoutGrid className="w-4 h-4"/></Button>
                <Button variant={viewMode === 'rows' ? 'secondary' : 'ghost'} size="icon" onClick={() => setViewMode('rows')}><Rows className="w-4 h-4"/></Button>
            </div>
        ...
        {viewMode === 'columns' ? (
          <DragDropContext onDragEnd={onDragEnd}>
            <div className="flex-1 flex gap-4 -mx-4 px-4 pb-4 overflow-x-auto">
              {stages.map(stage => (
                <StageColumn 
                  key={stage.id} 
                  stage={stage} 
