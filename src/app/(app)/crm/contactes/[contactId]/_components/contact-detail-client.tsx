...
import { ArrowLeft, User, Briefcase, FileText, Receipt, Activity as ActivityIcon, Edit, Save, Ban, Loader2 } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { format } from 'date-fns';
import { ca } from 'date-fns/locale';
import { updateContactAction } from './actions';
import { type Contact, type Quote, type Opportunity, type Invoice, type Activity } from '../page';

// --- Sub-components (internal to this file) ---
const StatusBadge: FC<{ status?: string | null }> = ({ status }) => {
    let colorClass = 'bg-muted text-muted-foreground';
    let text: string = status || 'N/A';
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
            <div className="flex-1">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                      {isEditing ? <Input name="nom" defaultValue={contact.nom} className="text-4xl font-bold p-0 border-0 bg-transparent focus-visible:ring-0" /> : <h1 className="text-4xl font-bold">{contact.nom}</h1>}
                      {isEditing ? <Input name="empresa" defaultValue={contact.empresa || ''} className="text-xl text-muted-foreground mt-1 p-0 border-0 bg-transparent focus-visible:ring-0" /> : <p className="text-xl text-muted-foreground mt-1">{contact.empresa}</p>}
                    </div>
                </div>
            </div>
                ...
                <TabsContent value="detalls" className="p-8 space-y-12">
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-8">
                            <div className="space-y-2"><Label>Email</Label>{isEditing ? (<Input name="email" type="email" defaultValue={contact.email || ''} />) : (<p className="text-lg pt-2">{contact.email || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Telèfon</Label>{isEditing ? (<Input name="telefon" defaultValue={contact.telefon || ''} />) : (<p className="text-lg pt-2">{contact.telefon || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Estat</Label>{isEditing ? (<Select name="estat" defaultValue={contact.estat}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Lead">Lead</SelectItem><SelectItem value="Actiu">Actiu</SelectItem><SelectItem value="Client">Client</SelectItem></SelectContent></Select>) : (<p className="text-lg pt-2">{contact.estat || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Càrrec</Label>{isEditing ? (<Input name="job_title" defaultValue={contact.job_title || ''} />) : (<p className="text-lg pt-2">{contact.job_title || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Sector</Label>{isEditing ? (<Input name="industry" defaultValue={contact.industry || ''} />) : (<p className="text-lg pt-2">{contact.industry || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Origen del Lead</Label>{isEditing ? (<Input name="lead_source" defaultValue={contact.lead_source || ''} />) : (<p className="text-lg pt-2">{contact.lead_source || '--'}</p>)}</div>
                        </div>
                    </div>
                    <div>
                        <h3 className="text-2xl font-bold mb-6">Informació Personal</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-8">
                            <div className="space-y-2"><Label>Aniversari</Label>{isEditing ? (<Input type="date" name="birthday" defaultValue={contact.birthday || ''} />) : (<p className="text-lg pt-2">{contact.birthday ? format(new Date(contact.birthday), 'dd/MM/yyyy') : '--'}</p>)}</div>
                            <div className="space-y-2"><Label>Ciutat</Label>{isEditing ? (<Input name="address.city" defaultValue={contact.address?.city || ''} />) : (<p className="text-lg pt-2">{contact.address?.city || '--'}</p>)}</div>
                            <div className="space-y-2"><Label>LinkedIn</Label>{isEditing ? (<Input name="social_media.linkedin" defaultValue={contact.social_media?.linkedin || ''} />) : (<p className="text-lg pt-2">{contact.social_media?.linkedin || '--'}</p>)}</div>
                    ...
                    <div>
                        ...
                        {isEditing ? (<Textarea name="notes" defaultValue={contact.notes || ''} rows={6} />) : (<p className="text-base text-muted-foreground whitespace-pre-wrap">{contact.notes || 'No hi ha notes per a aquest contacte.'}</p>)}
                    </div>
                </TabsContent>
            </Tabs>
        </div>
