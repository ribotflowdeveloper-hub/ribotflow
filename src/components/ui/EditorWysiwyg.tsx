...
import { cn } from '@/lib/utils/utils'; // Assegura't que tens aquest import
...
interface EditorWysiwygProps {
  ...
  className?: string; // ✅ 1. Acceptem className
}
...
export default function EditorWysiwyg({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          // L'estil 'prose' s'aplica ara a l'element d'edició intern
          'prose dark:prose-invert prose-sm sm:prose-base focus:outline-none min-h-[150px] max-w-none',
      },
    },
    // ✅ Canvi: Passem html i json a onChange
    onUpdate: ({ editor }) => {
      if (onChange) {
  ...
  return (
    <TooltipProvider delayDuration={100}>
      {/* ✅ 2. Contenidor principal ara és flex-col i aplica el className (que contindrà h-full) */}
      <div
        className={cn(
          'w-full rounded-md border border-input bg-transparent text-sm ring-offset-background',
          'flex flex-col', // Organització vertical
          className // Aplica l'alçada (h-full)
        )}
      >
        {/* ✅ 3. Barra d'eines 'sticky' i 'flex-shrink-0' */}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {['12px', '14px', '16px', '18px', '24px'].map((size) => (
                <DropdownMenuItem
                  key={size}
                  onSelect={() => editor.chain().focus().setMark('textStyle', { fontSize: size }).run()}
              ...
              <DropdownMenuItem onSelect={() => editor.chain().focus().setMark('textStyle', { fontSize: null }).run()}>
                Reset
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        {/* ✅ Contenidor de contingut amb mida fixa i scroll intern */}
