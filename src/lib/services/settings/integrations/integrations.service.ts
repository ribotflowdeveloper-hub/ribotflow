...
  supabase: SupabaseClient<Database>,
  user: User,
  activeTeamId: string, // ✅ CANVI 1: Rebem el 'activeTeamId'
...
    };
    
    // ✅ CANVI 2: Afegim els camps que falten a l'UPSERT
    const { error: dbError } = await supabase.from('user_credentials').upsert({ 
      user_id: user.id, 
      provider: 'custom_email',
      provider_user_id: email, // <-- AFEGIT: El correu de l'usuari
      team_id: activeTeamId,   // <-- AFEGIT: L'equip actiu
      config: configPayload, 
      encrypted_password: encryptedPassword,
      access_token: null, 
      refresh_token: null, 
      expires_at: null, 
    }, { onConflict: 'user_id, provider' }); // Aquesta 'onConflict' és correcta per a credencials d'usuari
    
...
    } else {
      // Per 'custom_email' i 'google_gmail', etc. esborra de 'user_credentials'
