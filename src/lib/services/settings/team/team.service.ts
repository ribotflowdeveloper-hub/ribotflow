...
// -----------------------------------------------------------------
// ❗️❗️ AQUESTA ÉS LA FUNCIÓ MODIFICADA ❗️❗️
// -----------------------------------------------------------------
/**
...
  try {
    // ✅ CANVI 1: Demanem el 'role' a més del 'team_id'
    const { data: member } = await supabase
      .from("team_members")
      .select("team_id, role") // <-- Demanem el rol
...
            active_team_plan: newTeamPlan,
            // ✅ CANVI 2: Afegim el rol de l'usuari al token (JWT)
            active_team_role: member.role,
          },
        },
      );
...
// -----------------------------------------------------------------
// ❗️❗️ AQUESTA ÉS L'ALTRA FUNCIÓ MODIFICADA ❗️❗️
// -----------------------------------------------------------------
/**
...
          active_team_plan: null,
            // ✅ CANVI 3: Netegem també el rol
            active_team_role: null,
        },
      },
    );
    if (error) throw error;
...
          active_team_id: null,
          active_team_plan: null,
            // ✅ CANVI 4 (OPCIONAL PERÒ RECOMANAT): Netegem també aquí
            active_team_role: null,
        },
      });
    }
...
    if (error) throw error;
    ...
    // ✅ CANVI 5 (OPCIONAL PERÒ RECOMANAT): Actualitzem el rol al token de l'usuari afectat
    // si aquest és l'equip que TÉ ACTIU.
    const supabaseAdmin = createAdminClient();
    const { data: { user: updatedUser } } = await supabaseAdmin.auth.admin.getUserById(memberUserId);

    if (updatedUser?.app_metadata?.active_team_id === activeTeamId) {
      await supabaseAdmin.auth.admin.updateUserById(memberUserId, {
        app_metadata: {
          ...updatedUser.app_metadata,
          active_team_role: newRole,
        },
      });
    }
