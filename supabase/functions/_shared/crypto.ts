...
// (Aquest és el codi que has proporcionat, lleugerament adaptat per a Deno)
import "https://deno.land/x/crypto@v0.2.1/mod.ts";
...
export async function decrypt(
  ...
  if (encryptedData.includes(":")) {
    try {
      ...
      throw new Error(
        "No s'ha pogut desxifrar el token (SubtleCrypto). La clau pot ser incorrecta o la dada corrupta.",
      );
    }
  }
  ...
  if (encryptedData.startsWith("U2FsdGVkX1")) {
    console.warn(
      "[CRYPTO-JS] S'està desxifrant un token antic (format CryptoJS).",
    );
    try {
      ...
      if (!decrypted) {
        throw new Error(
          "Dades desxifrades buides (CryptoJS). La clau pot ser incorrecta.",
        );
      }
      return decrypted;
    } catch (error) {
