...
// ✨ NOVES FUNCIONS DE CODIFICACIÓ ROBUSTES ✨
// Aquesta funció converteix correctament text amb caràcters especials a Base64 per a l'assumpte.
function encodeSubject(subject: string): string {
  const encoded = encodeBase64(new TextEncoder().encode(subject));
  return `=?UTF-8?B?${encoded}?=`;
}

// Aquesta funció converteix el cos del correu a Base64 URL-safe, com requereix l'API de Gmail.
...
serve(async (req) => {
  ...
  try {
    ...
    // PAS 1: Obtenir les dades del contacte
    ...
    // PAS 2: Obtenir l'usuari autenticat i les seves credencials de Google
    ...
    if (!creds?.refresh_token) throw new Error("No s'han trobat les credencials de Google.");

    // PAS 3: Obtenir un nou access token de Google
    ...
    // PAS 4: Construir i codificar el missatge del correu
    const emailMessage = [
      `Content-Type: text/html; charset="UTF-8"`,
      `MIME-Version: 1.0`,
      `To: ${contact.email}`,
      `From: ${user.email}`,
      `Subject: ${encodeSubject(subject)}`, // ✨ Utilitzem la nova funció
      ``,
      htmlBody
    ].join('\n');

    const rawEmail = encodeEmailForGmail(emailMessage); // ✨ Utilitzem la nova funció
