/**
 * @file index.ts (send-quote-pdf)
 * @summary Aquesta és una Supabase Edge Function que gestiona l'enviament d'un pressupost en format PDF
 * com a fitxer adjunt a un correu electrònic, utilitzant la integració de Gmail de l'usuari.
 * És una funció complexa que orquestra dades de diverses taules, Supabase Storage i l'API de Google.
 */
...
/**
 * @summary Codifica l'assumpte del correu a Base64 seguint el format RFC 2047 per a caràcters especials.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    // --- PAS 1: Obtenim totes les dades necessàries en una sola consulta ---
    const { data: quote, error: quoteError } = await supabaseAdmin
      ...
      // Validacions inicials per assegurar que tenim totes les dades necessàries.
    ...
    // Obtenim un nou 'access_token' de curta durada.
    ...
        // --- PAS 6: Construïm el correu en format MIME multipart/mixed ---
    // Aquest format és necessari per incloure text HTML i un fitxer adjunt en el mateix correu.
    ...
    const emailMessage = [
      ...
            ``,
            `--${boundary}`, // Inici de la primera part (HTML)
            `Content-Type: text/html; charset="UTF-8"`,
            ``,
            htmlBody,
            ``,
            `--${boundary}`, // Inici de la segona part (PDF adjunt)
      ...
            pdfBase64,
            ``,
            `--${boundary}--` // Final del missatge
          ].join('\n');
      
          const encodeEmailForGmail = (msg: string) => encodeBase64(new TextEncoder().encode(msg)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      
          // --- PAS 7: Enviar el correu a través de l'API de Gmail ---
      ...
          // --- PAS 8 i 9: Actualitzar l'estat del pressupost i de l'oportunitat ---
          await supabaseAdmin.from('quotes').update({ status: 'Sent', sent_at: new Date().toISOString() }).eq('id', quoteId);
          if (quote.opportunity_id) {
             await supabaseAdmin.from('opportunities').update({ stage_name: 'Proposta Enviada' }).eq('id', quote.opportunity_id);
          }
