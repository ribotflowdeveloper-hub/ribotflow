...
DECLARE 
    ...
    v_new_invoice_item_id BIGINT; -- CANVI: Assumim BIGINT per a ID, basant-nos en altres taules (Si el teu invoice_items.id és UUID, canvia-ho de nou)
...
CREATE OR REPLACE FUNCTION "public"."get_filtered_expenses"("p_team_id" "uuid", "p_search_term" "text" DEFAULT NULL::"text", "p_category_id" bigint DEFAULT NULL::bigint, "p_status" "text" DEFAULT NULL::"text", "p_sort_by" "text" DEFAULT 'expense_date'::"text", "p_sort_order" "text" DEFAULT 'desc'::"text", "p_limit" integer DEFAULT 10, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" bigint, "user_id" "uuid", "description" "text", "total_amount" numeric, "expense_date" "date", "created_at" timestamp with time zone, "invoice_number" "text", "tax_amount" numeric, "extra_data" "jsonb", "supplier_id" bigint, "subtotal" numeric, "discount_amount" numeric, "notes" "text", "team_id" "uuid", "status" "public"."expense_status", "payment_date" "date", "payment_method" "text", "is_billable" boolean, "project_id" bigint, "is_reimbursable" boolean, "supplier_nom" "text", "full_count" bigint, "retention_amount" numeric, "currency" "text", "due_date" "date", "category_id" bigint, "category_name" "text")
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
    ...
    WITH filtered_expenses AS (
        ...
        WHERE
            ...
            -- Hem de fer el CAST a text per compatibilitat amb el paràmetre p_status text
...
ALTER FUNCTION "public"."get_filtered_expenses"("p_team_id" "uuid", "p_search_term" "text", "p_category_id" bigint, "p_status" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_inbox_tickets"("p_visible_user_ids" "uuid"[], "p_search_term" "text", "p_active_filter" "text", "p_limit" integer, "p_offset" integer) RETURNS TABLE("id" bigint, "created_at" timestamp with time zone, "user_id" "uuid", "contact_id" bigint, "provider" "text", "provider_message_id" "text", "subject" "text", "body" "text", "sender_name" "text", "sender_email" "text", "sent_at" timestamp with time zone, "status" "text", "attachments" "jsonb", "preview" "text", "type" "text", "contact_nom" "text", "contact_email" "text", "profile_full_name" "text", "profile_avatar_url" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
    -- Aquest és el teu codi, que ja era correcte
    RETURN QUERY
    SELECT
        et.id,
        et.created_at,
...
BEGIN
    ...
    -- 2. Obtenir els Ítems amb les Taxes niades
    SELECT jsonb_agg(
...
CREATE OR REPLACE FUNCTION "public"."handle_onboarding"("p_user_id" "uuid", "p_full_name" "text", "p_email" "text", "p_company_name" "text", "p_tax_id" "text", "p_website" "text", "p_summary" "text", "p_sector" "text", "p_services" "jsonb", "p_phone" "text", "p_street" "text", "p_city" "text", "p_postal_code" "text", "p_region" "text", "p_country" "text", "p_latitude" numeric DEFAULT NULL::numeric, "p_longitude" numeric DEFAULT NULL::numeric) RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$DECLARE
  new_team_id uuid;
...
BEGIN
  ...
  -- 3.5. Inserir Pipeline per defecte
  INSERT INTO public.pipelines (team_id, name, "position")
  VALUES (new_team_id, 'Pipeline Principal', 1)
  RETURNING id INTO new_pipeline_id;

  -- 💡 CORRECCIÓ: PAS 4 (Inserim colors i tipus d'etapa)
  -- Afegeix les columnes 'color' i 'stage_type' a l'INSERT
  INSERT INTO public.pipeline_stages (
    name, "position", team_id, user_id, pipeline_id, color, stage_type
  )
  VALUES
    ('Prospecte', 1, new_team_id, p_user_id, new_pipeline_id, '#3b82f6', 'PROSPECT'),
    ('Contactat', 2, new_team_id, p_user_id, new_pipeline_id, '#06b6d4', 'CONTACTED'),
    ('Proposta Enviada', 3, new_team_id, p_user_id, new_pipeline_id, '#a855f7', 'PROPOSAL'),
    ('Negociació', 4, new_team_id, p_user_id, new_pipeline_id, '#eab308', NULL),
    ('Guanyat', 5, new_team_id, p_user_id, new_pipeline_id, '#22c55e', 'WON'),
    ('Perdut', 6, new_team_id, p_user_id, new_pipeline_id, '#ef4444', 'LOST');

  -- 5. Inserir Subscripció
  INSERT INTO public.subscriptions (team_id, plan_id, status)
  VALUES (new_team_id, 'plan_free', 'active');
  ...
  UPDATE auth.users
  SET 
    raw_user_meta_data = (user_metadata || jsonb_build_object('full_name', p_full_name)),
    raw_app_meta_data = (app_metadata || jsonb_build_object('active_team_id', new_team_id, 'active_team_plan', 'free'))
  WHERE id = p_user_id;
...
ALTER FUNCTION "public"."handle_onboarding"("p_user_id" "uuid", "p_full_name" "text", "p_email" "text", "p_company_name" "text", "p_tax_id" "text", "p_website" "text", "p_summary" "text", "p_sector" "text", "p_services" "jsonb", "p_phone" "text", "p_street" "text", "p_city" "text", "p_postal_code" "text", "p_region" "text", "p_country" "text", "p_latitude" numeric, "p_longitude" numeric) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."is_team_member"("team_id_to_check" "uuid") RETURNS boolean
  ...
  SELECT EXISTS (
    SELECT 1
    FROM public.team_members
    WHERE team_members.user_id = auth.uid() -- Ara auth.uid() funcionarà
...
CREATE OR REPLACE FUNCTION "public"."match_platform_documents"("query_embedding" "extensions"."vector", "match_threshold" real, "match_count" integer) RETURNS TABLE("id" bigint, "content" "text", "metadata" "jsonb", "similarity" real)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
    ...
    FROM
        public.platform_documents pd -- Ús de l'esquema explícit
...
ALTER FUNCTION "public"."match_platform_documents"("query_embedding" "extensions"."vector", "match_threshold" real, "match_count" integer) OWNER TO "postgres";
...
BEGIN
  ...
  IF v_quote.opportunity_id IS NOT NULL THEN
    BEGIN
      ...
      -- Pas 2.3: Trobar l’etapa marcada com LOST dins del mateix pipeline
      SELECT id INTO v_target_stage_id
      FROM public.pipeline_stages
      ...
      -- Pas 2.4: Si existeix, actualitzem l’oportunitat
      IF v_target_stage_id IS NOT NULL THEN
        UPDATE public.opportunities
        SET 
          pipeline_stage_id = v_target_stage_id,
          stage_name = 'Perdut' -- Actualitzem camp denormalitzat
        WHERE id = v_quote.opportunity_id;
      END IF;
...
CREATE OR REPLACE FUNCTION "public"."search_paginated_quotes"("team_id_param" "uuid", "status_param" "public"."quote_status" DEFAULT NULL::"public"."quote_status", "search_term_param" "text" DEFAULT NULL::"text", "sort_by_param" "text" DEFAULT 'issue_date'::"text", "sort_order_param" "text" DEFAULT 'desc'::"text", "limit_param" integer DEFAULT 10, "offset_param" integer DEFAULT 0) RETURNS TABLE("id" bigint, "sequence_number" integer, "user_id" "uuid", "contact_id" bigint, "team_id" "uuid", "show_quantity" boolean, "status" "public"."quote_status", "issue_date" "date", "expiry_date" "date", "subtotal" numeric, "discount_amount" numeric, "tax_amount" numeric, "retention_amount" numeric, "total_amount" numeric, "created_at" timestamp with time zone, "opportunity_id" bigint, "send_at" timestamp with time zone, "secure_id" "uuid", "sent_at" timestamp with time zone, "quote_number" "text", "notes" "text", "rejection_reason" "text", "theme_color" "text", "contact_nom" "text", "contact_empresa" "text", "total_count" bigint)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
BEGIN
    ...
    WITH filtered_quotes AS (
        SELECT
            ...
            q.discount_amount,
            q.tax_amount,
            -- q.tax_rate, -- <--- Comentari original que has d'eliminar si ja no hi és
            q.retention_amount,
            q.total_amount,
            q.created_at,
            q.opportunity_id,
...
ALTER FUNCTION "public"."search_paginated_quotes"("team_id_param" "uuid", "status_param" "public"."quote_status", "search_term_param" "text", "sort_by_param" "text", "sort_order_param" "text", "limit_param" integer, "offset_param" integer) OWNER TO "postgres";
...
ALTER TABLE "public"."platform_documents" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."platform_documents"
    ADD CONSTRAINT "platform_documents_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "platform_documents_embedding_idx" ON "public"."platform_documents" USING "ivfflat" ("embedding" "extensions"."vector_cosine_ops") WITH ("lists"='100');
...
ALTER TABLE ONLY "public"."blacklist_rules"
    ADD CONSTRAINT "blacklist_rules_team_id_fkey" FOREIGN KEY ("team_id") REFERENCES "public"."teams"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."profiles"
    ADD CONSTRAINT "profiles_active_team_id_fkey" FOREIGN KEY ("active_team_id") REFERENCES "public"."teams"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."quote_items"
    ADD CONSTRAINT "quote_items_team_id_fkey" FOREIGN KEY ("team_id") REFERENCES "public"."teams"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."social_posts"
    ADD CONSTRAINT "social_posts_team_id_fkey" FOREIGN KEY ("team_id") REFERENCES "public"."teams"("id") ON DELETE CASCADE;
...
CREATE POLICY "Admins i Owners poden crear invitacions" ON "public"."invitations" FOR INSERT TO "authenticated" WITH CHECK ((("team_id" = ((("auth"."jwt"() -> 'app_metadata'::"text") ->> 'active_team_id'::"text"))::"uuid") AND ((("auth"."jwt"() -> 'app_metadata'::"text") ->> 'active_team_role'::"text") = ANY (ARRAY['owner'::"text", 'admin'::"text"]))));
...
CREATE POLICY "Els membres poden ACTUALITZAR factures en 'Draft' del seu equip" ON "public"."invoices" FOR UPDATE TO "authenticated" USING ((("team_id" = "public"."get_active_team_id"()) AND ("status" = 'Draft'::"text"))) WITH CHECK (("team_id" = "public"."get_active_team_id"()));
...
CREATE POLICY "Members poden crear invitacions" ON "public"."invitations" FOR INSERT TO "authenticated" WITH CHECK ((((("auth"."jwt"() -> 'app_metadata'::"text") ->> 'active_team_role'::"text") = 'member'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."team_members"
  WHERE (("team_members"."user_id" = "auth"."uid"()) AND ("team_members"."team_id" = "invitations"."team_id"))))));
...
CREATE POLICY "Permetre accés complet al service_role per a job_postings" ON "public"."job_postings" FOR SELECT USING (("auth"."role"() = 'service_role'::"text"));



CREATE POLICY "Permetre accés complet al service_role per a teams" ON "public"."teams" FOR SELECT USING (("auth"."role"() = 'service_role'::"text"));



CREATE POLICY "Permetre lectura a usuaris autenticats de platform_documents" ON "public"."platform_documents" FOR SELECT TO "authenticated" USING (true);
...
GRANT ALL ON FUNCTION "public"."get_filtered_expenses"("p_team_id" "uuid", "p_search_term" "text", "p_category_id" bigint, "p_status" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_filtered_expenses"("p_team_id" "uuid", "p_search_term" "text", "p_category_id" bigint, "p_status" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_filtered_expenses"("p_team_id" "uuid", "p_search_term" "text", "p_category_id" bigint, "p_status" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."match_platform_documents"("query_embedding" "extensions"."vector", "match_threshold" real, "match_count" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."match_platform_documents"("query_embedding" "extensions"."vector", "match_threshold" real, "match_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."match_platform_documents"("query_embedding" "extensions"."vector", "match_threshold" real, "match_count" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."search_paginated_quotes"("team_id_param" "uuid", "status_param" "public"."quote_status", "search_term_param" "text", "sort_by_param" "text", "sort_order_param" "text", "limit_param" integer, "offset_param" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_paginated_quotes"("team_id_param" "uuid", "status_param" "public"."quote_status", "search_term_param" "text", "sort_by_param" "text", "sort_order_param" "text", "limit_param" integer, "offset_param" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_paginated_quotes"("team_id_param" "uuid", "status_param" "public"."quote_status", "search_term_param" "text", "sort_by_param" "text", "sort_order_param" "text", "limit_param" integer, "offset_param" integer) TO "service_role";
...
GRANT ALL ON TABLE "public"."platform_documents" TO "anon";
GRANT ALL ON TABLE "public"."platform_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."platform_documents" TO "service_role";
